...
describe('fetchCatsByOwner()', () => {
  test('invokes the request function', done => {
    ...
    function testCB() {
      expect(requestSpy).toHaveBeenCalledWith('/owners/calvin/cats', expect.any(Function));
      done();
    }
...
describe('fetchCatPics()', () => {
  ...
  test('handles error responses with a placeholder', done => {
    const cats = ['cute-cat', 'chonky-cat', 'scratchy-cat', 'pathetic-cat', 'out-of-place-dog'];
...
describe('fetchAllCats()', () => {
  ...
  test.only('invokes the callback with an array of all the cats, sorted in alphabetical order', done => {
...
describe('buySingleOutfit()', () => {
  test('invokes the request method', done => {
    function testCB() {
      expect(requestSpy).toHaveBeenCalledWith('/outfits/taco', expect.any(Function));
      done();
    }
