...
*/
const poundConvert = require('./utils/convertToPounds.js');
const sum = require('./utils/getArrayTotal.js');

function sumPurchases(item1, item2) {
  let item1Conversion = poundConvert(item1.cost, item1.exchangeRate);
  let item2Conversion = poundConvert(item2.cost, item2.exchangeRate);
  return Number((item1Conversion + item2Conversion).toFixed(2));
}
