...
runTest(
  ...
  function () {
    ...
    check(countProperties({ name: "Mezz" })).isEqualTo(1);
    check(
      countProperties({ name: "Mezz", job: "tutor", city: "Manchester" })
    ).isEqualTo(3);
  }
);
...
//Exercise 3
function createProfileDescription(obj) {
  /*
  ...
  If the 'likesToCode' property is true, then you should return a string of the format:
  ...
  */
  return obj.likesToCode === true ? `My name is ${obj.name} and I like to code.` : `My name is ${obj.name} and I don't like to code.`;
...
//Exercise 5
function howManyArguments(...theArgs) {
...
runTest(
  "updateCoinMachine() will create a message from a details object",
  function () {
    check(
...
function updatePosition(arr, str) {
 ...
 //not sure how to do this one with an object?
...
function checkGame(dice, coin) {
  ...
  return dice >= 3 && coin === 'H' ? true : false;
}
...
//Exercise 10
function addCoins(coinCollection, coinStr) {
  /*
  ...
  */
 if (coinStr === '1p') {
  coinCollection[0].push(coinStr)
 } else if (coinStr === '2p') {
  coinCollection[1].push(coinStr)
 } else if (coinStr === '5p') {
  coinCollection[2].push(coinStr)
 } else {
  coinCollection[3].push(coinStr);
 }
...
//   '10p' : 3,
//  }
// let coinIndex = coins[coinStr];
//  // compare to coin array
//  if (coinIndex === coinStr) { // how do I check if this is a number?
//   coinCollection[coinIndex].push(coinStr);
//  }
//  return coinCollection;
}
