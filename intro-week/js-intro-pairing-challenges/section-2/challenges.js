...
function checkSameFinalCharacter(str1, str2) {
  ...
  let strIndex1 = str1.length -1;
  let strIndex2 = str2.length -1;
  return str1.charAt(strIndex1) === str2.charAt(strIndex2);
}
...
function accessItem(arr, index) {
  ...
  // check index number against array.length if greater find remainder
console.log(index)
  if (index < arr.length){
    return arr[index];
  } else if (index >= arr.length){
    let remainder = index % arr.length;
...
function extractNumber(str) {
  ...
  // find first bracket
  //extract numbers until 2nd bracket
  // 
  // let regex = /^’(\d)’\$/

  let regex = /\((\d+)\)/
  console.log(str.match(regex))
  let number = str.match(regex); // gives back [(123), 123]
