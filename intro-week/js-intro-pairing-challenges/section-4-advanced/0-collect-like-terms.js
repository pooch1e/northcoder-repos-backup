...
function collectLikeTerms(stringExpression) {
  ...
  let stringArray = stringExpression.split('');
  // count amount of letters
  let letterCount = stringArray.reduce((acc, currentValue) => {
    (acc[currentValue] = acc[currentValue] || 0) + 1;
    return acc;
  }, {});
  console.log(letterCount);

  return `${Object.keys(letterCount).join('')}`;
}
