...
describe.skip('user_article_votes table', () => {
  ...
  test('user_article_votes table has user_article_votes_id column with a primary key of serial', () => {
    return db
      .query(
        `SELECT column_name, data_type, column_default
          FROM information_schema.columns
      ...
      .then(({ rows: [column] }) => {
        ...
        expect(column.data_type).toBe('integer');
        expect(column.column_default).toMatch(/nextval/);
      });
  });

  test('user_article_votes table has username column of varying character', () => {
    return db
      .query(
        `SELECT column_name, data_type
          FROM information_schema.columns
  ...
  test('user_article_votes table has vote_count column as integer and not null', () => {
    return db
      .query(
