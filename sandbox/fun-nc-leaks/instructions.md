...
Write a function called `getPeople` that will retrieve a list of all the available people on the `northcoders` server. This should:

1. Use Node's `https` module to make a request to `https://nc-leaks.herokuapp.com/api/people`.
2. Once you have the response as a usable object, look through the people to find anyone who has `northcoders` as the workplace.
...
Write a function called `getPets` that does the same as Task 2 but for pets. The endpoint is `https://nc-leaks.herokuapp.com/api/people/:username/pets`.

> Note: Some of the users do not have pets and so the server will give a 404 response! These responses should not be included in the `pets.json`. Consider how you will use the status code to inform when to include some pets.
...
Use a library such as [`superagent`](https://github.com/visionmedia/superagent) to make your requests easier to handle. Refactor your pets and interests requests to use superagent instead of Node's http module.
